...
fn analyze_factor(variables: &mut SymbolTable, parsed_factor: &ParsedFactor) -> Result<AnalyzedFactor, String> {
    match parsed_factor {
        ParsedFactor::Literal(value) => Ok(AnalyzedFactor::Literal(*value)),
        ParsedFactor::Identifier(name) => Ok(AnalyzedFactor::Identifier(variables.find_symbol(name)?)),
        ParsedFactor::SubExpression(expr) =>
            Ok(
                AnalyzedFactor::SubExpression(
                    Box::<AnalyzedExpr>::new(
...
fn analyze_term(variables: &mut SymbolTable, parsed_term: &ParsedTerm) -> Result<AnalyzedTerm, String> {
...
fn analyze_expr(variables: &mut SymbolTable, parsed_expr: &ParsedExpr) -> Result<AnalyzedExpr, String> {
...
fn analyze_statement(variables: &mut SymbolTable, parsed_statement: &ParsedStatement) -> Result<AnalyzedStatement, String> {
...
pub fn analyze_program(variables: &mut SymbolTable, parsed_program: &ParsedProgram) -> Result<AnalyzedProgram, String> {
